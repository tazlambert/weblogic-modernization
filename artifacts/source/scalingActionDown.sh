...
echo "scaling_action: $scaling_action" >> scalingActionDown.log
echo "wls_domain_uid: $wls_domain_uid" >> scalingActionDown.log
echo "wls_cluster_name: $wls_cluster_name" >> scalingActionDown.log
echo "wls_domain_namespace: $wls_domain_namespace" >> scalingActionDown.log
echo "operator_service_name: $operator_service_name" >> scalingActionDown.log
echo "operator_service_account: $operator_service_account" >> scalingActionDown.log
echo "operator_namespace: $operator_namespace" >> scalingActionDown.log
echo "scaling_size: $scaling_size" >> scalingActionDown.log
...
if [ $? -ne 0 ]
  then
    echo "Failed to retrieve status of $operator_service_name in name space: $operator_namespace" >> scalingActionDown.log
    echo "STATUS: $STATUS" >> scalingActionDown.log
    exit 1
fi
...
echo "port: $port" >> scalingActionDown.log
...
if [ $? -ne 0 ]
  then
    echo "Failed to retrieve Custom Resource Definition for WebLogic domain" >> scalingActionDown.log
    echo "CRD: $CRD" >> scalingActionDown.log
    exit 1
fi
...
echo "domain_api_version: $domain_api_version" >> scalingActionDown.log
...
if [ $? -ne 0 ]
  then
    echo "Failed to retrieve WebLogic Domain Custom Resource Definition" >> scalingActionDown.log
    echo "DOMAIN: $DOMAIN" >> scalingActionDown.log
    exit 1
fi
echo "DOMAIN: $DOMAIN" >> scalingActionDown.log
...
then
  echo "$wls_cluster_name defined in clusters" >> scalingActionDown.log
...
INPUT
  ...
  echo "$wls_cluster_name NOT defined in clusters" >> scalingActionDown.log
cat > cmds.py << INPUT
import sys, json
for i in json.load(sys.stdin)["items"]:
...
echo "current number of managed servers is $num_ms" >> scalingActionDown.log
...
echo "new_ms is $new_ms" >> scalingActionDown.log
...
echo "request_body: $request_body" >> scalingActionDown.log
...
then
  ...
  echo "Operator Cert File not found" >> scalingActionDown.log
  exit 1
fi
...
echo "operator_url: $operator_url" >> scalingActionDown.log
...
then
  ...
  echo "Operator PEM formatted file not found" >> scalingActionDown.log
  exit 1
fi

if [ $? -ne 0 ]
then
  echo "Failed scaling request to WebLogic Operator" >> scalingActionDown.log
  echo $result >> scalingActionDown.log
  exit 1
fi
echo $result >> scalingActionDown.log

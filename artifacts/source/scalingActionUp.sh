...
echo "scaling_action: $scaling_action" >> scalingActionUp.log
echo "wls_domain_uid: $wls_domain_uid" >> scalingActionUp.log
echo "wls_cluster_name: $wls_cluster_name" >> scalingActionUp.log
echo "wls_domain_namespace: $wls_domain_namespace" >> scalingActionUp.log
echo "operator_service_name: $operator_service_name" >> scalingActionUp.log
echo "operator_service_account: $operator_service_account" >> scalingActionUp.log
echo "operator_namespace: $operator_namespace" >> scalingActionUp.log
echo "scaling_size: $scaling_size" >> scalingActionUp.log
...
if [ $? -ne 0 ]
  then
    echo "Failed to retrieve status of $operator_service_name in name space: $operator_namespace" >> scalingActionUp.log
    echo "STATUS: $STATUS" >> scalingActionUp.log
    exit 1
fi
...
echo "port: $port" >> scalingActionUp.log
...
if [ $? -ne 0 ]
  then
    echo "Failed to retrieve Custom Resource Definition for WebLogic domain" >> scalingActionUp.log
    echo "CRD: $CRD" >> scalingActionUp.log
    exit 1
fi
...
echo "domain_api_version: $domain_api_version" >> scalingActionUp.log
...
if [ $? -ne 0 ]
  then
    echo "Failed to retrieve WebLogic Domain Custom Resource Definition" >> scalingActionUp.log
    echo "DOMAIN: $DOMAIN" >> scalingActionUp.log
    exit 1
fi
echo "DOMAIN: $DOMAIN" >> scalingActionUp.log
...
then
  echo "$wls_cluster_name defined in clusters" >> scalingActionUp.log
...
INPUT
  ...
  echo "$wls_cluster_name NOT defined in clusters" >> scalingActionUp.log
cat > cmds.py << INPUT
import sys, json
for i in json.load(sys.stdin)["items"]:
...
echo "current number of managed servers is $num_ms" >> scalingActionUp.log
...
echo "new_ms is $new_ms" >> scalingActionUp.log
...
echo "request_body: $request_body" >> scalingActionUp.log
...
then
  ...
  echo "Operator Cert File not found" >> scalingActionUp.log
  exit 1
fi
...
echo "operator_url: $operator_url" >> scalingActionUp.log
...
then
  ...
  echo "Operator PEM formatted file not found" >> scalingActionUp.log
  exit 1
fi

if [ $? -ne 0 ]
then
  echo "Failed scaling request to WebLogic Operator" >> scalingActionUp.log
  echo $result >> scalingActionUp.log
  exit 1
fi
echo $result >> scalingActionUp.log
